...
vecPackage=c("lubridate","data.table")
...
get_sample_weather <- function(dsample=NULL,first_year=NULL,last_year=NULL,time_windows=c(0,3,9),
                               ...
                               nc_data="table_weather_rds_names.csv",
                               nc_ref_file="normal_weather_precipitation_mean_temp_1950-2000.rds",
                               nc_rep="C:/git/data_weather/",
...
##library(lubridate)
  ...
  ##  dsample=dsample;first_year=NULL;last_year=NULL;time_windows=c(0,3,9);nc_local=TRUE;nc_extract=FALSE;nc_data="C:/git/Vigie-Chiro_scripts/data_weather/table_weather_Rdata_names.csv";nc_rep="C:/git/Vigie-Chiro_scripts/data_weather";nc_ref_file="C:/git/Vigie-Chiro_scripts/data_weather/normal_weather_precipitation_mean_temp_Inf--Inf.Rdata";var=c("precipitation","mean_temp");dsample_colnames=c("site_id"="site_id","date"="date","longitude"="longitude","latitude"="latitude");output=TRUE;save=FALSE;fileoutput=NULL
    ...
    nc_data <- paste0(nc_rep,nc_data)
    nc_ref_file <- paste0(nc_rep,nc_ref_file)
    ...
    dsample$julian_day  <- yday(dsample$date)
    ...
    while(i <=nrow(dsample)){
        ...
        for(l in 1:length(lnc)) {
            ...
            point.TM$julian_day  <- yday(point.TM$date_extract)
            ...
            while (year_ii <= end_year & ii <= nrow(dsample)) {
                ...
                for(tw in time_windows) {
                    dates <- as_date((as_date(date_j0) - tw):(as_date(date_j0)))
                    ...
                    match_ref_date <- which(lnc_mean[[v]]$julian_day %in% dates_julian)
                    ...
                    weather[[l]][["raw"]][ii,name_tw] <- val_raw
                    weather[[l]][["anomaly"]][ii,name_tw] <- val_anomalie
                    weather[[l]][["anomaly_st"]][ii,name_tw] <- val_anomalie_st
                    weather[[l]][["ref_mean"]][ii,name_tw] <- ref_mean
                    weather[[l]][["ref_sd"]][ii,name_tw] <- ref_sd
    ...
    dweather <- data.frame(row = 1:nrow(dsample))
    for(l in length(weather)) {
        for(ll in 1:4) {
            ddweather <- weather[[l]][[ll]]
            colnames(ddweather) <-paste(colnames(ddweather),names(weather)[l],names(weather[[l]])[ll],sep="_")
            dweather <- cbind(dweather,ddweather)
        }}


    dweather <- dweather[,-1]

  ##  tc1 <- paste0(rep(c("raw","ano","ano_st"),each=length(time_windows)),"_tw",rep(time_windows,lenght=4))
  ##  rtc1 <- rep(tc1,length(var))
  ##  theColnames <- paste(rep(var,each= length(tc1)),rtc1,sep="_")

  ##  colnames(dweather) <- theColnames

    dsample <- cbind(dsample,dweather)
...
##' @author Romain Lorrilliere
##' @importFrom climateExtract extract_nc_value
nc2rds <- function(firstYear=1950,lastYear=NULL,nc_version="22.0e",repOut="C:/git/data_weather") {
                                        #  firstYear=1950;lastYear=NULL;repOut="data/"
    library(climateExtract) #https://github.com/RetoSchmucki/climateExtract
    ...
    for(i in 1:nrow(dAn)) {
        cat("\n",paste(dAn[i,],collapse=" | "),"\n\n")
        flush.console()
        cat("\n - precipitation: avec le fichier rr_ens_mean_0.25deg_reg_v",nc_version,".nc\n",sep="")
        flush.console()
        precipitation <- extract_nc_value(dAn$start_real[i],dAn$end[i]) # avec le fichier rr_ens_mean_0.25deg_reg_v20.0e.nc
        ...
        cat("\n - mean_temp : avec le fichier tg_ens_mean_0.25deg_reg_v",nc_version,".nc\n",sep="")
        flush.console()
        mean_temp <- extract_nc_value(dAn$start_real[i],dAn$end[i]) # avec le fichier tg_ens_mean_0.25deg_reg_v20.0e.nc
...
assess_weather_ref <- function(first_year=1950,last_year=2000,
                               ...
                               nc_extract=FALSE,
                               nc_data="C:/git/data_weather/table_weather_rds_names.csv",
                               nc_rep="C:/git/data_weather",
